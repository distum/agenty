		...
		TextToText(openai.TextToTextParams{
			...
			FuncDefs: []openai.FuncDef{
				// function without parameters
				{
					...
					Body: func(ctx context.Context, _ []byte) (agenty.Message, error) {
						// because we don't need any arguments
						return agenty.NewTextMessage(agenty.ToolRole, "42"), nil
					},
				},
				// function with parameters
				{
					...
					Body: func(ctx context.Context, params []byte) (agenty.Message, error) {
...
- Assistant: 20 and 42`)
	...
	answer, err := t2tOp.Execute(
		ctx,
		agenty.NewMessage(agenty.UserRole, agenty.TextKind, []byte("what is the meaning of life?")),
	)
	if err != nil {
		panic(err)
	}
	...
	answer, err = t2tOp.Execute(
		ctx,
		agenty.NewMessage(agenty.UserRole, agenty.TextKind, []byte("1+1?")),
	)
	if err != nil {
		panic(err)
	}
	...
	answer, err = t2tOp.Execute(
		ctx,
		agenty.NewMessage(agenty.UserRole, agenty.TextKind, []byte("1+1 and what is the meaning of life?")),
	)
	if err != nil {
		panic(err)
	}
...
func printAnswer(message agenty.Message) {
	fmt.Printf(
